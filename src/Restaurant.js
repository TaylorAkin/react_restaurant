import React from 'react';
import './Restaurant.css';


// class HelloMessage extends React.Component {
//   render() {
//     return (
//       <div>
//         Hello Taylor
//       </div>
//     );
//   }
// }

class Restaurant extends React.Component {

    render() {

        return (
            <React.Fragment>

            <Navbar />
            <div>

            </div>
            <Footer />
                
            </React.Fragment>
  
    )

    }

}


function Navbar() {
    return (
        <nav className="navbar navbar-expand-lg navbar-dark">
            <button className="navbar-toggler navbar-toggler-icon" type="button" data-toggle="collapse" data-target="#navbarTogglerDemo03" aria-controls="navbarTogglerDemo03" aria-expanded="false" aria-label="Toggle navigation" />

            <a className="navbar-brand" href="#">Stellas</a>

            <div className="collapse navbar-collapse" id="navbarTogglerDemo03">
                <ul className="navbar-nav mr-auto mt-2 mt-lg-0">
                    <li className="nav-item active">
                        <a className="nav-link" href="#">Home <span className="sr-only">(current)</span></a>
                    </li>
                    <li className="nav-item active">
                        <a className="nav-link" href="#">Link</a>
                    </li>
                    <li className="nav-item active">
                        <a className="nav-link" href="#" tabindex="-1" aria-disabled="true">Disabled</a>
                    </li>
                </ul>
                <form className="form-inline my-2 my-lg-0">
                    <input className="form-control mr-sm-2" type="search" placeholder="Search" aria-label="Search" />
                    <button className="btn btn-outline-success my-2 my-sm-0" type="submit">Search</button>
                </form>
            </div>
        </nav>
    );
}

function Footer() {

    return (
        <div className='footer fixed-bottom'>

            <div className='container'>

                <div className = 'row'>

                    <small className = 'col-3'>Stellas</small>

                    <small className = 'col-3'>8334 W. Trusel Avenue
                Spring Valley, CA 91977</small>

                    <small> Monday-Thursday <br /> 
                    10:45-10PM      <br />
                    Friday & Saturday <br />
                    10:45-11PM
                
                </small>

                </div>

            </div>

        </div>
    )

}


export default Restaurant;
